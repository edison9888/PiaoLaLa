...
#import "DzOrderDetailViewController.h"
#import "AlipayConnect.h"
#import "NSStringAdditions.h"
#import "UIImageViewSite.h"
@implementation DzOrderDetailViewController
...
-(void)loadView{
    ...
    NSLog(@"orderTicket %@",[kAppDelegate.temporaryValues objectForKey:@"STREAK"]);
    
    self.typeLabel.text = [[kAppDelegate.temporaryValues objectForKey:@"STREAK"] 
                           objectForKey:@"FILMTYPE"];
    ...
    //    STREAKPLAYTIME HALLNAME
    self.timeLabel.text = [[kAppDelegate.temporaryValues objectForKey:@"STREAK"] 
                           objectForKey:@"STREAKPLAYTIME"];
    
    self.hallLabel.text = [[kAppDelegate.temporaryValues objectForKey:@"STREAK"] 
                           objectForKey:@"HALLNAME"];
    NSString *_site = @"";
    for (UIImageViewSite *site in [kAppDelegate.temporaryValues objectForKey:@"orderSelects"]) {
        _site = [NSString stringWithFormat:@"%@ %@排%@座",
         _site,[site.userInfo objectForKey:@"SEATX"],
         [site.userInfo objectForKey:@"SEATY"]];
    }
    self.siteLabel.text = _site;
...
-(IBAction)buyConfirm{
    ...
    [_order setObject:[[kAppDelegate.temporaryValues objectForKey:@"STREAK"] 
                       objectForKey:@"FILMID"]
               forKey:@"FILMID"];
    
    [_order setObject:[[kAppDelegate.temporaryValues objectForKey:@"STREAK"] 
                       objectForKey:@"STREAKID"]
               forKey:@"STREAKID"];
    
    [_order setObject:[NSString stringWithFormat:@"%d",
                       [[kAppDelegate.temporaryValues 
                         objectForKey:@"orderSelects"] count]]
               forKey:@"TICKETNUM"];
    
    [_order setObject:[[kAppDelegate.temporaryValues 
                        objectForKey:@"orderSelects"] valueForKey:@"userInfo"]
               forKey:@"SEATLIST"];
...
-(void)textFieldDidBeginEditing:(UITextField *)textField{
    self.back.frame = CGRectMake(self.back.frame.origin.x, -130, self.back.frame.size.width, self.back.frame.size.height);
}
